...
function load_config(name)
	local status_ok, config = require(string.format("config/%s", name))
	if not status_ok then
		return
	else
		return config
...
return packer.startup(function(use)
    ...
    use { "kyazdani42/nvim-tree.lua", config=load_config("nvim-tree") } -- File explorer
	...
	use { "kyazdani42/nvim-web-devicons", config=load_config('web-devicons') }
